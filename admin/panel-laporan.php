...
// Ambil data klik_laporan + nomor pengacara (telepon)
$resultKlik = $conn->query("
    SELECT k.*, p.nama AS pengacara_nama, p.spesialis AS pengacara_spesialis, p.telepon AS pengacara_nomor
    FROM klik_laporan k
    LEFT JOIN pengacara p ON k.pengacara_id = p.id
    ORDER BY k.id DESC
");
              ...
              <?php while ($row = $resultKlik->fetch_assoc()): ?>
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-3 py-2 border space-x-2">
                    ...
                    <?php if (!empty($row['pengacara_nomor'])): ?>
                      <?php 
                        // Normalisasi nomor WA
                        $nomor = preg_replace('/[^0-9]/', '', $row['pengacara_nomor']);
                        if (substr($nomor, 0, 1) === "0") {
                            $nomor = "62" . substr($nomor, 1);
                        }

                        $pesan = urlencode(
                          "Assalamualaikum Bapak/Ibu " . $row['pengacara_nama'] . ",\n\n" .
                          "Ada user *" . $row['user_nama'] . "* yang sedang menunggu jawaban " .
                          "dari konsultasi hukum *" . $row['jenis_konsultasi'] . "*.\n\n" .
                          "Mohon untuk segera menanggapi pertanyaan:\n" .
                          "\"" . $row['pertanyaan'] . "\""
                        );
                        $waLink = "https://wa.me/" . $nomor . "?text=" . $pesan;
                      ?>
                      <a href="<?= $waLink ?>" target="_blank" 
                         class="text-green-600 hover:underline">Verifikasi</a>
                    <?php else: ?>
                      <span class="text-gray-400">No HP tidak tersedia</span>
                    <?php endif; ?>
                  </td>
                </tr>
              <?php endwhile; ?>
